...
Use tRPC v11 async-generator subscriptions. The procedure's `signal` is aborted when the renderer cancels the operation, navigates, or closes.
...
Legacy Observable subscriptions are also accepted for migration compatibility.

### Transformers

Client transformers belong on the IPC link in tRPC v11:
...
The main-process router must be initialized with the same transformer.
